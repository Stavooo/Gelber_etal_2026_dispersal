...
| Figs. 5--7 | habitat x disturbance x autocorrelation (modified, fragmentation 0.7) | `370` | -- |
...
| Figs. S7--S10 | log-normal vs exponential kernel comparison (S10 uses the 4-variable Figs 5--7 runs, fragmentation 0.1/0.9 pooled) | `231` | `230` |
...
at disturbance 0; S2 and S4 are the analogous fragmentation slices); run `370`
also provides the main-text Figures 5--7. The kernel-comparison figures S7--S9
are assembled from the cached summaries of the runs above; Figure S10
additionally uses the four-variable runs `231`/`230` (fragmentation 0.1 and 0.9
pooled).
